...
namespace rknn {
    ...
    private:
        std::vector<std::shared_ptr<Tensor>> inputs_;  // 会在build_engine_input_and_outputs_mapper与rknn_input绑定
        std::vector<std::shared_ptr<Tensor>> outputs_;  // 会在build_engine_input_and_outputs_mapper与rknn_output绑定
    ...
    bool InferImpl::load_from_memory(const void* pdata, size_t size, bool sync_mode) {
        ...
        return build_engine_input_and_outputs_mapper();  // 对输入输出的指针地址进行绑定，以及获取输入输出信息
    }
    ...
    bool InferImpl::build_engine_input_and_outputs_mapper() {
        ...
        if(!checkRKNN(rknn_query(rknn_handle_, RKNN_QUERY_IN_OUT_NUM, &inout_num, sizeof(inout_num)))){  // 获取输入输出数量
        ...
        for(int i = 0; i < inout_num.n_input; ++i){
            ...
            input.index = i;
            input.buf = newTensor->cpu();  // 将输入的指针地址与Tensor进行绑定
        ...
        for(int i = 0; i < inout_num.n_output; ++i){
            ...
            output.buf = newTensor->cpu();  // 将输出指针地址与Tensor进行绑定
