...
#if defined(U_OS_LINUX) || defined(U_OS_ANDROID)
...
namespace kiwi{
    ...
    void __log_func(const char* file, int line, LogLevel level, const char* fmt, ...) {
...
#if defined(U_OS_WINDOWS) || defined(U_OS_ANDROID)
        ...
        if (level == LogLevel::Fatal) {
            __android_log_print(ANDROID_LOG_FATAL, "kiwi", "%s", buffer);
        }
        else if(level == LogLevel::Error){
            __android_log_print(ANDROID_LOG_ERROR, "kiwi", "%s", buffer);
        }
        else if (level == LogLevel::Warning) {
            __android_log_print(ANDROID_LOG_WARN, "kiwi", "%s", buffer);
        }
        else {
            __android_log_print(ANDROID_LOG_INFO, "kiwi", "%s", buffer);
        }
    #else
